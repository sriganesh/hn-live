...
type Tab = 'feed' | 'following' | 'tags' | 'profile' | 'bookmarks';
...
export function UserFeedPage({ 
  ...
  // Add tab order array
  const TAB_ORDER: Tab[] = ['feed', 'following', 'profile', 'bookmarks', 'tags'];
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => {
                    const timestamp = Math.floor(Date.now() / 1000);
                    const content = JSON.stringify(feedItems, null, 2);
                    const blob = new Blob([content], { type: 'application/json' });
                    const url = URL.createObjectURL(blob);
                    const a = document.createElement('a');
                    a.href = url;
              ...
              {feedItems.length === 0 ? (
                <div className="text-center py-8 opacity-75">
                  ...
                  <div className="mt-2 text-sm">Bookmarks are private to you and stored locally in your browser.</div>
                </div>
              ) : (
                feedItems.map(item => (
                  <div key={item.id} className="space-y-2">
